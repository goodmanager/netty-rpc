...
public class WeightedResponseTimeStrategy<T> implements ProviderStrategy<T> {
	@Override
	public ServiceInstance<T> getInstance(InstanceProvider<T> instanceProvider) throws Exception {
		...
		Server selectServer = LoadBalancerUtil.selectServer(hosts, SelectStrategy.WEIGHTEDRESPONSETIME);
		for (ServiceInstance<T> instance : instances) {
