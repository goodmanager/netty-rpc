				...
				new InvocationHandler() {
					...
					public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
						...
						if (registerCenterConfig.getRegisterCenterType().getIndex() == RegisterCenterType.ZOOKEEPER
							...
							ConsulClient consulClient = RegisterCenterUtil.getConsulClient(registerCenterConfig);
							ConsulServiceDiscovery consulServiceDiscovery = new ConsulServiceDiscovery(consulClient);
							Response<List<HealthService>> healthResponse = consulServiceDiscovery
									.findServices(interfaceName);
							...
							if (healthResponse == null) {
								...
								logger.info("requestId:{},发现服务完毕,准备解析服务[{}]", requestId, healthResponse);
								// 解析服务地址
								logger.info("requestId:{},服务地址解析完毕,准备构建RPC客户端", requestId);
								// 构建rpc客户端
								Service service = healthResponse.getValue().get(0).getService();
